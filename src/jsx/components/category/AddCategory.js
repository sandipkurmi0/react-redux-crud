...
import { addCategory, updateCategory } from "../../../stores/category/actions";
const AddCategory = (props) => {
  const history = useHistory();
  const [btnLoading, setBtnLoading] = useState(false);
  const [pageName, setPageName] = useState("Add Category");
  ...
  const addCategory = async () => {
    ...
    if (props.match.params.id) {
      await props.updateCategory(state, props.match.params.id);
    } else {
      await props.addCategory(state);
    }
          ...
          <div className="p-3">
            <div className="overflow-x-auto">
              <TextInput
                ...
                label="Categoryname"
                type="text"
                onchangeInput={(e) => setState({ ...state, Categoryname: e })}
                autofocus
                autocapitalize="off"
